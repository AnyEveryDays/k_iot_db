...
#		>> 테이블명(별칭).열이름 형식으로 표기!
# Error Code: 1052. Column `'member_id' in where clause is ambigous(where 절이 애매하다)
...
from `purchases` P
	join `members`M
    on P.member_id = M.member_id # SQL 일치
where
	amount >= 20000;
